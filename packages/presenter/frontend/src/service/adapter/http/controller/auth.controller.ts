...
export class AuthController {
  ...
  constructor(
    @inject(TYPES.ApiBaseUrl) private readonly baseUrl: string,
  ) {
  ...
  async getUserById(userId: string): Promise<User> {
    try {
      const response = await this.httpClient.get<ApiResponse<User>>(`/${userId}`);
  ...
  async updatePassword(passwordData: UpdatePasswordRequest): Promise<void> {
    try {
      ...
      console.info(
        `Password updated successfully for: ${passwordData.email}`
      );
    } catch (error) {
  ...
  async validateCredentials(credentials: LoginRequest): Promise<User> {
    try {
      ...
      console.info(`Login successful: ${credentials.email}`);

      return response.data as User;
    } catch (error) {
      console.error(`Login failed: ${credentials.email}`);
      throw error;
    }
  }
}
